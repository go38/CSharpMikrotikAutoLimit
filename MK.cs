...
namespace MikrotikAutoLimitter
{
    internal class MK
    {
        private readonly TcpClient con;
                ...
                else
                {
                    ...
                    else
                    {
                        if (tmp[3] < 0xE0)
                        {
                            ...
                            int tmpi = BitConverter.ToInt32(
                                new byte[] {(byte) connection.ReadByte(), tmp[2], tmp[3], 0}, 0);
                            count = tmpi ^ 0xC00000;
                        }
                        else
                        {
                            if (tmp[3] < 0xF0)
                            {
                                ...
                                int tmpi =
                                    BitConverter.ToInt32(new[] {(byte) connection.ReadByte(), tmp[1], tmp[2], tmp[3]}, 0);
        ...
        public string EncodePassword(string Password, string hash)
        {
            ...
            {
                hash_byte[i/2] = Byte.Parse(hash.Substring(i, 2), NumberStyles.HexNumber);
            }
